  ...
  return (
    <header className='static sm:sm-left-0 sm:z-50 sm:mt-32 mx-auto'>
      <div className='sm:flex sm:flex-col sm:justify-between sm:text-end'>
        <UnstyledLink href='/' className='font-bold text-3xl hidden sm:block transition-all ease-in-out duration-600 hover:text-[#4FB464] active:text-[#A0A0A0] sm:mb-16'>
          {'~/basara'}
        </UnstyledLink>
        <nav>
          <ul className='flex sm:flex-col justify-between text-end pt-2 sm:pt-0'>
            {links.map(({ href, label }) => (
              <li key={`${href}${label}`} className='sm:mb-16 text-xl p-2'>
                <UnstyledLink href={href} className={router.pathname===href?'text-[#4FB464] motion-safe:animate-pulse':'' + ' transition-all ease-in-out text-white duration-600 active:text-[#A0A0A0] hover:text-[#4FB464]'}>
                  {label}
                </UnstyledLink>
              </li>
            ))}
